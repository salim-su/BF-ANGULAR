...
export class StartupService {
  constructor(
    ...
    iconSrv.fetchFromIconfont({
      scriptUrl: 'https://at.alicdn.com/t/font_1707145_o2s3up9lv2.js',
    });
  }
  ...
  private viaHttp(resolve: any, reject: any) {
    zip(
      ...
      this.httpClient.get('blade-system/menu/routes?subsystemId=' + environment.subsystem.plateform.id),
      this.httpClient.get('blade-system/menu/buttons?subsystemId=' + environment.subsystem.plateform.id),
    ).pipe(
      catchError(([user, subsystem, menus, buttons]) => {
        resolve(null);
        return [user, subsystem, menus, buttons];
      }),
    ).subscribe(([user, subsystem, menus, buttons]) => {
